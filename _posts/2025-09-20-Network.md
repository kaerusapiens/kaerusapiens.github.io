...
sequenceDiagram
    ...
    Client->>Server: GET /login HTTP/1.1<br/>Host: example.com<br/>(쿠키 없음)
    Server->>Server: 사용자 인증 처리<br/>로그인 성공
    Server->>Client: HTTP/1.1 200 OK<br/>Set-Cookie: sessionid=abc123xyz<br/>Set-Cookie: user_pref=dark_mode<br/>Set-Cookie: last_visit=2025-09-23<br/>(쿠키 저장 지시)
    Note over Client: 브라우저가 쿠키 저장<br/>(도메인별로 관리)

    Note over Client,Server: 2단계: 이후 요청 (쿠키 자동 전송)
    Client->>Server: GET /dashboard HTTP/1.1<br/>Host: example.com<br/>Cookie: sessionid=abc123xyz;<br/>user_pref=dark_mode;<br/>last_visit=2025-09-23
    Server->>Server: 쿠키 파싱 및 세션 확인<br/>sessionid로 사용자 식별
    Server->>Client: HTTP/1.1 200 OK<br/>(대시보드 데이터)
```
...
sequenceDiagram
    participant Client as 클라이언트<br/>(브라우저)
    participant Server as 웹 서버
    participant SessionStore as 세션 저장소<br/>(메모리/DB/Redis)
    ...
    Server->>SessionStore: 세션 생성 요청<br/>(사용자 정보 저장)
    SessionStore-->>Server: 세션 ID 생성<br/>session_id=xyz789abc
    Note over Server: 세션 데이터 저장<br/>{session_id: "xyz789abc",<br/>user_id: 123,<br/>username: "user1",<br/>login_time: "2025-09-23 10:00:00"}
    Server->>Client: HTTP/1.1 200 OK<br/>Set-Cookie: session_id=xyz789abc;<br/>Path=/; HttpOnly; Secure
    ...
    Server->>Client: HTTP/1.1 200 OK<br/>(프로필 데이터)
    ...
    Server->>Client: HTTP/1.1 200 OK<br/>Set-Cookie: session_id=; Expires=Thu, 01 Jan 1970 00:00:00 GMT
```
